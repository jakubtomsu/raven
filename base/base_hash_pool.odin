...
Hash_Pool :: struct($N: int, $D: typeid, $H: typeid) where intrinsics.type_core_type(H) == Handle {
    gen:    [N]Handle_Gen,
    hash:   [N]u64,
    data:   [N]D,
}
...
@(require_results)
hash_pool_find_free :: proc "contextless" (pool: $T/Hash_Pool($N, $D, $H), hash: u64) -> (H, bool) {
    assert(hash != 0)
    for offs in 0..<HASH_POOL_MAX_PROBE_DIST {
        slot := (hash + offs) %% N
        if slot != 0 && pool.hash[slot] == 0 {
...
@(require_results)
hash_pool_remove :: proc(pool: ^$T/Hash_Pool($N, $D, $H), handle: Handle) -> bool {
    if !hash_pool_has(pool^, handle) {
        return false
    }
...
@(require_results)
hash_pool_get :: proc(pool: ^$T/Hash_Pool($N, $D, $H)) -> (^D, bool) {
    if !hash_pool_has(pool^, handle) {
        return nil, false
    }
